/******************************************************************************
  ...
  financeira HP12c, utilizando os conceitos de Estrutura de Dados de Pilha (Stack) para o processamento de expressões em Notação Polonesa Reversa (RPN).
  Autor(a): Christian Moreira Santos
  Data: 31/03/2026
...
int main() {
    ...
    // Separando a espressão em partes pelo delimitador de espaco
    char *expressao_em_partes = strtok(expressao, " ");

    // Usamos While para percorres todas as entradas simulando o input do usuario
    while (expressao_em_partes != NULL) {
        printf("\nEntrada: '%s'\n", expressao_em_partes);

        // Validação se a posição atual é um digito ou caractere
